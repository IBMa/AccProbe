...
public class MouseCursorAction extends Action
{
	...
	public MouseCursorAction (IWorkbenchWindow window, String label) {
	...
	public void run () {
		...
		if (accService != null && accService instanceof MsaaAccessibilityService) {
			if (isChecked()) {
				if(getText().equals("&Mouse cursor")){
					if(!view.registerMouseEventListener()){
						setChecked(false);
					}
				}
			}else {
				if(getText().equals("&Mouse cursor")){
					view.removeMouseEventListener();
				}
			}
		}
	}
